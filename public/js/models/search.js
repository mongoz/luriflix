window.Employee = Backbone.Model.extend({
    ...
    sync: function(method, model, options) {
        if (method === "read") {
            store.findById(parseInt(this.id), function (data) {
...
window.EmployeeCollection = Backbone.Collection.extend({
    ...
    sync: function(method, model, options) {
        if (method === "read") {
            store.findByName(options.data.name, function (data) {
                options.success(data);
            });
        }
    }
...
window.ReportsCollection = Backbone.Collection.extend({

    model: Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            store.findByManager(this.parent.id, function (data) {
                options.success(data);
            });
        }
    }
...
window.MemoryStore = function (successCallback, errorCallback) {
